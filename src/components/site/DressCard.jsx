  ...
  return (
    <div className="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition-shadow hover:shadow-md">
      <div className="relative aspect-[4/3] bg-gray-50">
        {item.image?.url ? (
          <Image src={item.image.url} alt={item.name} fill className="object-cover transition-transform duration-500 group-hover:scale-105" sizes="360px" />
        ) : (
          <span className="absolute inset-0 grid place-items-center text-gray-200"><Gem className="h-9 w-9" /></span>
        )}
        {item.isDress && <span className="absolute left-3 top-3 rounded-full bg-brand-500 px-2.5 py-0.5 text-xs font-medium text-white">Dress</span>}
      </div>
      <div className="p-4">
        ...
        <button onClick={() => onBook(item)} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-brand-500 py-2 text-sm font-medium text-white transition-colors hover:bg-brand-600">
